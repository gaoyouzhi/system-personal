...
public class SM2Factory {
	...
	public  byte[] sm2GetZ(byte[] userId, ECPoint userKey){
		...
		byte[] p = HexByteUtil.byteConvert32Bytes(this.a);
		sm3.update(p, 0, p.length);

		p = HexByteUtil.byteConvert32Bytes(this.b);
		sm3.update(p, 0, p.length);

		p = HexByteUtil.byteConvert32Bytes(this.gx);
		sm3.update(p, 0, p.length);

		p = HexByteUtil.byteConvert32Bytes(this.gy);
		sm3.update(p, 0, p.length);

		p = HexByteUtil.byteConvert32Bytes(userKey.normalize().getXCoord().toBigInteger());
		sm3.update(p, 0, p.length);

		p = HexByteUtil.byteConvert32Bytes(userKey.normalize().getYCoord().toBigInteger());
		sm3.update(p, 0, p.length);
