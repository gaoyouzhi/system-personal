package com.haochang.encry.sm2;


import com.haochang.encry.util.HexByteUtil;
...
public class SM2EncDecUtils {
    ...
    public static SM2KeyVO generateKeyPair(){
        ...
        //System.out.println("公钥: " + HexByteUtil.byteToHex(publicKey.getEncoded()));
        //System.out.println("私钥: " + HexByteUtil.byteToHex(privateKey.toByteArray()));
        return sm2KeyVO;
    }
...
//      System.out.println("C1 " + HexByteUtil.byteToHex(c1.getEncoded()));
//      System.out.println("C2 " + HexByteUtil.byteToHex(source));
//      System.out.println("C3 " + HexByteUtil.byteToHex(c3));
        //C1 C2 C3拼装成加密字串
        // C1 | C2 | C3
        //return HexByteUtil.byteToHex(c1.getEncoded()) + HexByteUtil.byteToHex(source) + HexByteUtil.byteToHex(c3);
        // C1 | C3 | C2
        return HexByteUtil.byteToHex(c1.getEncoded()) + HexByteUtil.byteToHex(c3) + HexByteUtil.byteToHex(source);
    }
    ...
    public static byte[] decrypt(byte[] privateKey, byte[] encryptedData) throws IOException
    {
        ...
        String data = HexByteUtil.byteToHex(encryptedData);
        /***分解加密字串 C1 | C2 | C3
         * （C1 = C1标志位2位 + C1实体部分128位 = 130）
         * （C3 = C3实体部分64位  = 64）
         * （C2 = encryptedData.length * 2 - C1长度  - C2长度）

        byte[] c1Bytes = HexByteUtil.hexToByte(data.substring(0,130));
        int c2Len = encryptedData.length - 97;
        byte[] c2 = HexByteUtil.hexToByte(data.substring(130,130 + 2 * c2Len));
        byte[] c3 = HexByteUtil.hexToByte(data.substring(130 + 2 * c2Len,194 + 2 * c2Len));
        */
        /***分解加密字串 C1 | C3 | C2
         * （C1 = C1标志位2位 + C1实体部分128位 = 130）
         * （C3 = C3实体部分64位  = 64）
        ...
        byte[] c1Bytes = HexByteUtil.hexToByte(data.substring(0,130));
        int c2Len = encryptedData.length - 97;
        byte[] c3 = HexByteUtil.hexToByte(data.substring(130,130 + 64));
        byte[] c2 = HexByteUtil.hexToByte(data.substring(194,194 + 2 * c2Len));
...
/*    public static BigInteger[] Sm2Sign(byte[] md, AsymmetricCipherKeyPair keypair)
    {
        SM3Digest com.haochang.encry.sm3 = new SM3Digest();
        ...
        com.haochang.encry.sm3.update(z, 0, z.length);

        byte[] p = md;
        com.haochang.encry.sm3.update(p, 0, p.length);

        byte[] hashData = new byte[32];
        com.haochang.encry.sm3.doFinal(hashData, 0);
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        String cipherText = SM2EncDecUtils.encrypt(HexByteUtil.hexToByte(pubk), sourceData);
        ...
        plainText = new String(SM2EncDecUtils.decrypt(HexByteUtil.hexToByte(prik), HexByteUtil.hexToByte(cipherText)));
