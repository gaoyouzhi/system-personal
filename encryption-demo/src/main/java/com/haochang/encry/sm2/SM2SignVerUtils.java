...
public class SM2SignVerUtils {
	...
	public static SM2SignVO Sign2SM2(byte[] privatekey,byte[] sourceData) throws Exception{
		...
		sm2SignVO.setSm3_z(HexByteUtil.getHexString(z));
		sm2SignVO.setSign_express(HexByteUtil.getHexString(sourceData));

		sm3Digest.update(z, 0, z.length);
		sm3Digest.update(sourceData,0,sourceData.length);
		byte [] md = new byte[32];
		sm3Digest.doFinal(md, 0);
		//System.out.println("SM3摘要值: " + HexByteUtil.getHexString(md));
		sm2SignVO.setSm3_digest(HexByteUtil.getHexString(md));
		...
		String result = HexByteUtil.byteToHex(sign.getEncoded());
	...
	public static SM2SignVO VerifySignSM2(byte[] publicKey,byte[] sourceData,byte[] signData){
		try {
			...
			SM3Digest sm3Digest = new SM3Digest();
			byte [] z = factory.sm2GetZ(USER_ID.getBytes(), userKey);
			//System.out.println("SM3摘要Z: " + HexByteUtil.getHexString(z));
			verifyVo.setSm3_z(HexByteUtil.getHexString(z));
			sm3Digest.update(z,0,z.length);
			sm3Digest.update(sourceData,0,sourceData.length);
			byte [] md = new byte[32];
			sm3Digest.doFinal(md, 0);
			//System.out.println("SM3摘要值: " + HexByteUtil.getHexString(md));
			verifyVo.setSm3_digest(HexByteUtil.getHexString(md));
	...
	public static void main(String[] args) throws Exception {
		...
		SM2SignVO sign = SM2SignVerUtils.Sign2SM2(HexByteUtil.hexStringToBytes(privatekey), sourceData);
		SM2SignVO verify = SM2SignVerUtils.VerifySignSM2(HexByteUtil.hexStringToBytes(publicKey), sourceData, HexByteUtil.hexStringToBytes(sign.getSm2_signForSoft()));
